...
const BubbleSort = () => {
  ...
  const [highlightMode, setHighlightMode] = useState('swaps');
    ...
    for (let i = 0; i < n - 1; i++) {
      for (let j = 0; j < n - i - 1; j++) {
        ...
        newSteps.push({
          ...
          comparisons: comparisons,
          swaps: swaps,
          explanation: `Comparing ${arr[j]} and ${arr[j + 1]} at indices ${j} and ${j + 1}.`,
          line: 4
        });
      ...
      newSteps.push({
        ...
        comparisons: comparisons,
        swaps: swaps,
        explanation: `Pass ${i + 1} completed. Element at index ${n - i - 1} is now in its sorted position.`,
        line: i === n - 2 ? 10 : 2
      });
    }
  ...
  useEffect(() => {
    if (steps.length > 0 && currentStep < steps.length) {
      ...
      setSwappingIndices(step.swapped ? step.compared : []);
    ...
    <div className="pt-22 min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 p-6 overflow-x-hidden relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold text-white mb-2">Bubble Sort Algorithm</h1>
          ...
          <div className="flex items-end justify-center h-64" style={{ gap: '2px' }}>
            {array.map((value, index) => {
              ...
              if (sortedIndices.has(index)) {
                ...
                (highlightMode === 'swaps' || highlightMode === 'both') && 
